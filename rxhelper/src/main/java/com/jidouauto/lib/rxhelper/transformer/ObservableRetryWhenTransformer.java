...
class ObservableRetryWhenTransformer<T> implements ObservableTransformer<T, T>,
    ...
    public ObservableRetryWhenTransformer(RetryOnError retryOnError, int retryCount, long delayMillisecond, ObservableSource<?> retryAfter) {
                    ...
                    if (mRetryOnError.isRetry(throwable)) {
                        ...
                        if (currentRetry < mRetryCount) {
                            ...
                            if (mRetryAfter == null) {
                                ...
                                return Flowable.timer(mDelayMillisecond, TimeUnit.MILLISECONDS).flatMap(i -> new ObservableFirstSingle<>(mRetryAfter).toFlowable());
                            }
                        }
                    }
                    ...
                    if (mRetryOnError.isRetry(throwable)) {
                        ...
                        if (currentRetry < mRetryCount) {
                            ...
                            if (mRetryAfter == null) {
                                ...
                                return Observable.timer(mDelayMillisecond, TimeUnit.MILLISECONDS).flatMap(i -> mRetryAfter);
                            }
                        }
                    }
